...
<display:table pagesize="5" class="displaytag" keepStatus="false"
	...
	<jstl:choose>
		<jstl:when
			test="${row.coordinates.latitude != null && row.coordinates.longitude != null}">
			<acme:columnOut code="rendezvous.coordinates"
				path="${row.coordinates.latitude }, ${row.coordinates.longitude }" />
		</jstl:when>
		<jstl:otherwise>
	...
	<jstl:if test="${isListingCreated == false}">
		<acme:columnOut code="rendezvous.creator"
	...
	<acme:columnButton url="rendezvous/display.do?rendezvousId=${row.id}"
	...
	<security:authorize access="hasRole('USER')">
				...
				<jstl:if test="${row.draft == true}">
					<jstl:if test="${row.deleted == false}">
						<jstl:if test="${row.moment > now}">
							<acme:button url="rendezvous/user/edit.do?rendezvousId=${row.id}"
		...
		<display:column>
			<jstl:if test="${row.creator == principal}">
				<acme:button
					url="announcement/user/create.do?rendezvousId=${row.id}"
					code="rendezvous.addAnnouncement" />
			</jstl:if>
		</display:column>
		...
		<display:column>
			<jstl:if test="${row.deleted == false}">
				<jstl:if test="${row.moment > now}">
					<jstl:if test="${!row.attendants.contains(principal)}">
						<acme:button url="question/user/display.do?rendezvousId=${row.id}"
							code="rendezvous.RSVP" />
					</jstl:if>
				</jstl:if>
			</jstl:if>
			...
			<jstl:if test="${isListingRSVPd == true}">
				<jstl:if test="${row.attendants.contains(principal)}">
					<jstl:if test="${row.moment > now}">
						<acme:button
							url="rendezvous/user/cancel.do?rendezvousId=${row.id}"
...
<jstl:if test="${isListingRSVPd == true}">
	<jstl:if test="${row.attendants.contains(principal)}">
		<acme:button
			url="announcement/user/listRSVP.do?userId=${principal.id}"
			code="rendezvous.listAnnouncements" />
	</jstl:if>
</jstl:if>
